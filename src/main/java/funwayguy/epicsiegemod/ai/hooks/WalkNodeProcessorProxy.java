...
public class WalkNodeProcessorProxy extends WalkNodeProcessor
{
    ...
    private PathPoint getSafePoint(int x, int y, int z, int p_186332_4_, double p_186332_5_, EnumFacing facing)
    {
        ...
        if(entity != null)
        {
	        ...
	        if(bBounds1 == null || (bBounds1.maxX - bBounds1.minX) < 0.5F || (bBounds1.maxZ - bBounds1.minZ) < 0.5F)
	        {
	        	...
	        	{
		        	// Platform too small and risky to cross safely (accounts for open trapdoor case)
		        	return null;
	        	}
	        }
        }
        ...
        else
        {
            ...
            } else if(blockstate instanceof BlockFluidBase && BlockFluidBase.getFlowDirection(blockaccess, blockpos) >= 0D)
            {
            	// Flowing liquids are bad
            	pathnodetype = PathNodeType.BLOCKED;
            } else if(blockstate.getBlock() instanceof BlockLiquid && m_getFlow != null)
            {
            	try
            	{
    ...
    public PathNodeType getPathNodeType(IBlockAccess accessIn, int x, int y, int z)
    {
        ...
        {
            Block block = accessIn.getBlockState(new BlockPos(x, y - 1, z)).getBlock();
        ...
        if (pathnodetype == PathNodeType.WALKABLE)
        {
            for (int j = -1; j <= 1; ++j)
            {
                for (int i = -1; i <= 1; ++i)
                {
                    if (j != 0 || i != 0)
                    {
                        Block block1 = accessIn.getBlockState(blockpos$pooledmutableblockpos.setPos(j + x, y, i + z)).getBlock();
    ...
    static
    {
    	...
    	} catch(Exception e1)
    	{
    		...
    		{
    			ESM.logger.log(Level.ERROR, "Unable to hook into fluid flow direction", e2);
    		}
    	}
    }
}
